...
- Decision: keep the popover (positioned borderless window). Native menu is used for the right-click-style quick actions is not available (one click event only), so all actions live in the popover plus the settings window.
