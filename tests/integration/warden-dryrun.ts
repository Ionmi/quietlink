...
const firstToken = r.token;
check(r.ok && typeof r.token === "number" && state().tookDown === true, "hold persists tookDown before down");
check(await until(() => state().tookDown === false, 5000), "lease expiry restores AWDL");
check((await request({ op: "renew", token: firstToken })).error === "lease-expired", "stale token rejected");
...
check((await request({ op: "ping" })).ok, "ping answered from status snapshot");
